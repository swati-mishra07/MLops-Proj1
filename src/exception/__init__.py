...
def error_message_detail(error: Exception, error_detail: sys) -> str:
    ...
    # Create a formatted error message string with file name,
    # line number, and the actual error
    line_number = exc_tb.tb_lineno
    error_message = (
        f"Error occurred in python script: [{file_name}] at line number "
        f"[{line_number}]: {str(error)}"
    )
...
class MyException(Exception):
    ...
    def __init__(self, error_message: str, error_detail: sys):
        ...
        # Format the detailed error message using the
